...
def find_all_employees_data(sheet):
    ...
    for row in sheet.iter_rows(min_row=1, values_only=True):
        if row[0] and isinstance(row[0], str) and 'Empr.:' in row[0]:
            ...
            # Procura por 'Líquido:' em qualquer coluna e pega o valor na próxima coluna
            for i, cell in enumerate(row):
                if isinstance(cell, str) and 'Líquido:' in cell:
                    current_employee['Líquido'] = row[i+1] if i+1 < len(row) and row[i+1] is not None else None
                    break
